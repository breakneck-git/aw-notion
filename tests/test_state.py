...
def test_save_and_reload(tmp_path):
    ...
    state.last_sync = datetime(2026, 4, 11, 22, 0, tzinfo=UTC)
    state.notion_entries["abc123"] = {
        "page_id": "notion-page-id-xyz",
        "created_at": datetime(2026, 4, 11, 21, 30, tzinfo=UTC).isoformat(),
    }
    state.save(path)

    reloaded = State.load(path)
    assert reloaded.last_sync == datetime(2026, 4, 11, 22, 0, tzinfo=UTC)
...
def test_save_atomic_keeps_original_on_write_failure(tmp_path, monkeypatch):
    ...
    state.last_sync = datetime(2030, 1, 1, tzinfo=UTC)
...
def test_load_migrates_old_format(tmp_path):
    path = tmp_path / "state.json"
    path.write_text('{"last_sync": null, "notion_entries": {"oldsig": "old-page-id"}}')
    state = State.load(path)
